...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_evaluation'])) {
    ...
    if ($score < 1 || $score > 5) {
        $_SESSION['error_msg'] = 'กรุณาให้คะแนนประเมินระหว่าง 1-5';
    } else {
        try {
            ...
            if ($existingEvaluation) {
                ...
                if ($currentRequest && $currentRequest['Re_status'] == '2') {
                    ...
                    $_SESSION['success_msg'] = 'ระบบตรวจสอบพบข้อมูลประเมินเดิม และได้อัปเดตสถานะเป็น "ประเมินแล้ว" เรียบร้อยครับ';
                } else {
                    $_SESSION['error_msg'] = 'คุณได้ประเมินข้อร้องเรียนนี้ไปแล้ว ไม่สามารถแก้ไขได้';
                }
            } else {
                ...
                if (!$request) {
                    $_SESSION['error_msg'] = 'ไม่พบข้อร้องเรียนที่ระบุ หรือข้อร้องเรียนยังไม่เสร็จสิ้น';
                } else {
                    $evaluationData = [
                        'Eva_score' => $score,
                        'Eva_sug' => $suggestion,
                    ...
                    if ($result) {
                        $db->update('request', ['Re_status' => '3'], 'Re_id = ?', [$requestId]);
                        $_SESSION['success_msg'] = 'ส่งการประเมินเรียบร้อย ขอบคุณสำหรับความคิดเห็น';
                    } else {
                        $_SESSION['error_msg'] = 'เกิดข้อผิดพลาดในการบันทึกข้อมูลการประเมิน';
                    }
                }
            }
        } catch (Exception $e) {
...
<body>
    ...
    <script>
        ...
        function handleNotificationClick(notificationId, requestId) {
            ...
            if (requestId) window.location.href = `detail.php?id=${requestId}`;
        }
