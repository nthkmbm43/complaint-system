...
safeInclude('../../models/Auth.php', 'auth');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['register']) && $dbConnected) {
    try {
        ...
        // บันทึกข้อมูล (ใช้ hashed password และ Unit_id)
        $insertData = [
            'Stu_id' => $studentId,
            'Stu_name' => $fullName,
            'Stu_password' => password_hash($password, PASSWORD_DEFAULT),
...
<head>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .blob-1, .blob-2 {
        position: absolute; filter: blur(80px); z-index: -1; opacity: 0.4; border-radius: 50%;
        animation: float 10s infinite alternate;
    }
    .blob-1 { width: 400px; height: 400px; background: #6C63FF; top: -50px; left: -100px; }
    .blob-2 { width: 500px; height: 500px; background: #48C9B0; bottom: -100px; right: -100px; animation-delay: -5s; }
    @keyframes float { 0% { transform: translateY(0) scale(1); } 100% { transform: translateY(50px) scale(1.1); } }
    
    .container { width: 100%; max-width: 800px; margin: 0 auto; z-index: 10; }
    ...
    .header p { font-size: 1rem; opacity: 0.7; }

    .register-body { padding: 30px 40px 40px; }

    .status-info { border-radius: 12px; padding: 15px; margin-bottom: 25px; text-align: center; }
    .status-info.error { background: rgba(255, 107, 107, 0.1); border: 1px solid rgba(255, 107, 107, 0.3); color: #ff6b6b; }
    .status-info.warning { background: rgba(241, 196, 15, 0.1); border: 1px solid rgba(241, 196, 15, 0.3); color: #f1c40f; }

    .form-title { text-align: center; margin-bottom: 30px; }
    .form-title h2 { color: #fff; font-size: 1.6rem; margin-bottom: 5px; font-weight: 500; }
    .form-title p { color: rgba(255,255,255,0.6); font-size: 0.95rem; }
    ...
    .form-group select option { background: #1e1e2e; color: #fff; }
    .form-group input::placeholder { color: rgba(255, 255, 255, 0.3); }
    .form-group input:focus, .form-group select:focus {
        outline: none; border-color: #6C63FF; box-shadow: 0 0 0 4px rgba(108, 99, 255, 0.2); background: rgba(0, 0, 0, 0.3);
    }
    ...
    .input-hint { font-size: 0.85rem; color: rgba(255,255,255,0.5); margin-top: 5px; }

    .select-with-icon { position: relative; display: flex; align-items: center; gap: 10px; }
    .select-with-icon select { flex: 1; }
    .selected-icon {
        display: flex; align-items: center; justify-content: center;
        width: 50px; height: 50px; background: rgba(255,255,255,0.1);
        border-radius: 12px; font-size: 1.5rem; flex-shrink: 0;
        border: 1px solid rgba(255,255,255,0.1);
        color: #fff;
    }
    .selected-icon.empty::after { content: '❓'; opacity: 0.5; }

    .btn {
        background: linear-gradient(45deg, #6C63FF, #48C9B0); color: white;
        padding: 15px 30px; border: none; border-radius: 12px; font-size: 1.1rem;
        font-weight: 500; cursor: pointer; transition: all 0.3s ease;
        display: inline-flex; align-items: center; justify-content: center; gap: 8px;
        width: 100%; box-shadow: 0 4px 15px rgba(108, 99, 255, 0.3);
        margin-top: 10px; text-decoration: none;
    }
    .btn:hover { transform: translateY(-2px); box-shadow: 0 8px 25px rgba(108, 99, 255, 0.5); }
    .btn:disabled { background: rgba(255,255,255,0.1); color: rgba(255,255,255,0.3); cursor: not-allowed; box-shadow: none; transform: none; }
    ...
    .back-link { text-align: center; margin-top: 25px; padding-top: 20px; border-top: 1px solid rgba(255,255,255,0.05); }
    .back-link a { color: #a5b4fc; text-decoration: none; font-size: 0.95rem; display: inline-flex; align-items: center; gap: 5px; transition: all 0.3s ease; }
    .back-link a:hover { color: #fff; transform: translateY(-2px); }
