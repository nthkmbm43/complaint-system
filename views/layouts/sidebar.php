...
<?php
$studentPageTitles = [
    'index' => 'หน้าหลัก Dashboard',
    'complaint' => 'ส่งข้อร้องเรียน',
    'tracking' => 'ติดตามสถานะ',
    'evaluation' => 'ประเมินความพึงพอใจ',
    'profile' => 'ข้อมูลส่วนตัว',
    'edit_complaint' => 'แก้ไขข้อร้องเรียน',
    'detail' => 'รายละเอียดข้อร้องเรียน'
];
$stTitle = $studentPageTitles[$currentPage] ?? 'ระบบข้อร้องเรียน';
?>
<header class="top-header">
    <div class="header-left">
        ...
        <div class="header-title">
            ...
            <p><?php echo $stTitle; ?></p>
        </div>
    </div>
    <div class="header-right">
        <div class="header-notification" id="notificationButton" onclick="toggleNotificationDropdown()">
            <span style="font-size: 18px;">🔔</span>
            <span class="notification-badge<?php echo (isset($unreadCount) && $unreadCount > 0) ? '' : ' zero'; ?>" id="notificationBadge"><?php echo $unreadCount ?? 0; ?></span>
        ...
        <div class="user-menu" id="userProfileMenu" onclick="toggleProfileDropdown()">
            ...
            <div class="profile-dropdown" id="profileDropdown">
                <a href="index.php"><span class="icon">🏠</span> หน้าหลัก</a>
                <a href="complaint.php"><span class="icon">📝</span> ส่งข้อร้องเรียน</a>
                <a href="tracking.php"><span class="icon">📋</span> ติดตามสถานะ</a>
                <a href="evaluation.php"><span class="icon">⭐</span> ประเมินความพึงพอใจ</a>
                <div class="divider"></div>
                <a href="profile.php"><span class="icon">👤</span> ข้อมูลส่วนตัว</a>
                <div class="divider"></div>
                <a href="../../logout.php" class="logout"><span class="icon">🚪</span> ออกจากระบบ</a>
            </div>
        </div>
    </div>
</header>
<style>
    ...
    .header-title h1 { font-size: 1.2rem; margin: 0; color: #333; }
    ...
    /* Notification Bell */
    .header-notification {
        position: relative; padding: 10px; background: #f8f9fa;
        border-radius: 50%; cursor: pointer; transition: all 0.3s ease;
        width: 45px; height: 45px; display: flex; align-items: center; justify-content: center;
    }
    .header-notification:hover { background: #e9ecef; transform: scale(1.05); }
    .notification-badge {
        position: absolute; top: -2px; right: -2px; background: #dc3545;
        color: white; border-radius: 50%; width: 20px; height: 20px;
        font-size: 11px; display: flex; align-items: center; justify-content: center;
        font-weight: bold; min-width: 20px; animation: pulse 2s infinite;
    }
    ...
    .notification-dropdown {
        ...
        max-height: 400px; background: white; border-radius: 12px;
        box-shadow: 0 8px 25px rgba(0, 0, 0, 0.15); opacity: 0;
        visibility: hidden; transform: translateY(-10px); transition: all 0.3s ease;
        z-index: 1001; overflow: hidden; text-align: left;
    }
    ...
    .notification-header h3 { margin: 0; font-size: 1rem; color: #333; }
    .mark-all-read {
        background: none; border: none; color: #667eea; cursor: pointer;
        font-size: 0.85rem; padding: 5px 10px; border-radius: 5px; transition: all 0.3s ease;
    }
    ...
    .profile-dropdown {
        ...
        transition: all 0.3s ease; z-index: 1001; overflow: hidden; padding: 10px 0;
    }
    .profile-dropdown.show { opacity: 1; visibility: visible; transform: translateY(0); }
    .profile-dropdown a {
        display: flex; align-items: center; padding: 12px 20px; color: #333;
        text-decoration: none; transition: background 0.2s; font-size: 0.95rem;
    }
    .profile-dropdown a:hover { background: #f8f9fa; color: #667eea; }
    .profile-dropdown .icon { margin-right: 15px; font-size: 1.1rem; width: 20px; text-align: center; }
    .profile-dropdown .divider { height: 1px; background: #e9ecef; margin: 8px 0; }
    .profile-dropdown .logout { color: #dc3545; }
    .profile-dropdown .logout:hover { background: #fff5f5; color: #dc3545; }
</style>
<script>
...
function toggleNotificationDropdown() {
        ...
        if (notiDropdown) {
            ...
            if (notiDropdown.classList.contains('show')) {
                loadNotifications(); // Reload when opened
            }
        }
    }

    function loadNotifications() {
        fetch('../../api/notifications.php?ajax=1&action=get_notifications&limit=10')
            .then(res => res.json())
            .then(data => {
                if (data.success) {
    ...
    function renderNotifications(notifications) {
        ...
        if (!notifications || notifications.length === 0) {
            list.innerHTML = '<div class="no-notifications">ไม่มีการแจ้งเตือนใหม่</div>';
            return;
        }
        ...
        notifications.forEach(noti => {
            ...
            item.className = 'notification-item' + (noti.Noti_status == 0 ? ' unread' : '');
            
            // Navigate to detail.php when clicked
            item.onclick = function(e) {
                e.stopPropagation();
                fetch(`../../models/Notification.php?ajax=1&action=mark_as_read&notification_id=${noti.Noti_id}`)
                    .then(() => {
                        if (noti.Re_id) {
                            window.location.href = `detail.php?id=${noti.Re_id}`;
                        } else {
                            loadNotifications();
                        }
                    });
            ...
            item.innerHTML = `
                <div class="notification-title">${noti.Noti_title}</div>
                <div class="notification-message">${noti.Noti_message}</div>
                <div style="font-size: 0.75rem; color: #999; margin-top: 5px;">${noti.time_ago}</div>
            `;
    ...
    function markAllAsRead() {
        fetch('../../api/notifications.php?ajax=1&action=mark_all_as_read')
            .then(res => res.json())
            .then(data => {
                if (data.success) {
                    loadNotifications();
    ...
    document.addEventListener('click', function(e) {
        ...
        if (userMenu && profileDropdown && !userMenu.contains(e.target)) {
            profileDropdown.classList.remove('show');
        }

        const notiBtn = document.getElementById('notificationButton');
        const notiDropdown = document.getElementById('notificationDropdown');
        if (notiBtn && notiDropdown && !notiBtn.contains(e.target)) {
...
<div class="sidebar-overlay" id="sidebarOverlay" onclick="if(typeof toggleSidebar === 'function') toggleSidebar(); else if(typeof closeSidebar === 'function') closeSidebar(); else this.classList.remove('show'); document.getElementById('sidebar')?.classList.remove('show');"></div>

<div class="sidebar" id="sidebar">
    <div class="sidebar-header">
        <div class="sidebar-logo" style="padding: 5px;">
            <img src="../../assets/img/rmuti_logo.png" alt="RMUTI Logo" style="width: 100%; height: 100%; object-fit: contain; filter: drop-shadow(0 2px 4px rgba(0,0,0,0.2));">
        </div>
...
<script>
    ...
    function navigateWithAccessControl(url, requiredRole, requiredPermission) {
        ...
        if (!isLoggedIn) {
            ...
            window.location.href = '../../views/students/login.php?redirect=' + encodeURIComponent(window.location.pathname);
            return;
        }
