    ...
    public function login($identifier, $password, $requiredRole = null)
    {
        try {
            ...
            if (password_verify($password, $user['password'])) {
                ...
                // plaintext fallback for old accounts → pass
            } else {
                if (defined('DEBUG_MODE') && DEBUG_MODE) {
    ...
    public function registerStudent($data)
    {
        try {
            ...
            $insertData = [
                ...
                'Stu_password' => password_hash($data['password'], PASSWORD_DEFAULT),
    ...
    public function createTeacherAccount($data)
    {
        try {
            ...
            $insertData = [
                ...
                'Aj_password' => password_hash($data['password'], PASSWORD_DEFAULT),
        ...
        if (!$this->isLoggedIn()) {
            if ($redirectTo === null) {
                ...
                if (strpos($currentPath, '/staff/') !== false || strpos($currentPath, '/admin/') !== false) {
                    $redirectTo = 'login.php?message=session_expired';
